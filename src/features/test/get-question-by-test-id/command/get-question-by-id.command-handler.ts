...
export class GetQuestionByIdCommandHandler {
    ...
    public async execute(testId: string) {
        ...
        const multipleChoiceQuestionData = await this.multipleChoiceQuestionRepository.find({
            where: {
                test: {
                    id: testId
                }
        ...
        return this.mergeAndSortTwoArrayByOrder(oneChoiceQuestionData, multipleChoiceQuestionData);
    }

    private mergeAndSortTwoArrayByOrder(arr1: OneChoiceQuestion[], arr2: MultipleChoiceQuestion[]): Question[] {
