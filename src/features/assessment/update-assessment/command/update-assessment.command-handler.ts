...
import { removeUndefinedAttribute } from 'src/common/remove-undefined-attribute';
...
export class UpdateAssessmentCommandHandler {
    ...
    public async execute(id: string, command: UpdateAssessmentCommand) {
        ...
        if (command.testIds) {
            ...
            const missingTestIds = command.testIds.filter((testId: string) => !foundTestIds.includes(testId));
            ...
            const updatedAssessment = await this.assessmentRepository.save(existAssessment);
