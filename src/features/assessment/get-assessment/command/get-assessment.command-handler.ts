...
export class GetAssessmentCommandHandler {
    public async execute(command: GetAssessmentCommand) {
        ...
        const data = await this.findAllAssessments(skip, size, camelToSnakeCase(sortBy), direction, name, status);
    ...
    async findAllAssessments(skip: number, take: number, sortBy: string, direction: string, name?: string, status?: AssessmentStatus[]) {
    ...
    async getCount(name?: string, status?: AssessmentStatus[]): Promise<number> {
        ...
        if (Array.isArray(status) && status.length > 0) {
            conditions.push(`a.status IN (${status.map((s) => `'${s}'`).join(', ')})`);
        }
